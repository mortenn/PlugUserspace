(function() {
	...
	var senpai = {
		parseResult: function(result) {
			if (!result) {
				//senpai.cache[result.id].message = 'Unknown song or alternate version.';
				if (senpai.pos !== undefined)
					return;
				API.chatLog("Your next song is not in our systems, please ensure it follows all rules and is not overplayed. (Other videos of the same song might be banned.) http://s.AnimeMusic.me/plug-guide", false);
			} else if (result.b == 1) {
				senpai.cache[result.id].message = '<em style="color:red">Banned: ' + result.r + '</em>.';
				API.chatLog("Your next song is banned for '" + result.r + "', please choose a different song.", true);
				if (senpai.pos < 6) {
					window.noisy.trigger("Your next song is banned for '" + result.r + "', please choose a different song.");
				}
			} else if (result.rp > 0) {
				senpai.cache[result.id].message = '<em style="color:red">Already played in your last 6 turns.</em>';
				API.chatLog("You have played that song within your past 6 dj turns, please choose a different song.", true);
				if (senpai.pos < 5) {
					window.noisy.trigger('You have played that song within your past 6 dj turns, please choose a different song.');
				}
			} else if (result.o == 1) {
				senpai.cache[result.id].message = 'Overplayed.';
				API.chatLog("Your next song is overplayed, please choose a different song.", true);
				if (senpai.pos < 5) {
					window.noisy.trigger('Your next song is overplayed, please choose a different song.');
				}
			} else if (result.t == 1) {
				senpai.cache[result.id].message = 'Played today.';
				API.chatLog("Your next song has already been played in the last 24 hours, please choose a different song.", true);
				if (senpai.pos < 4) {
					window.noisy.trigger('Your next song has already been played in the last 24 hours, please choose a different song.');
				}
			} else if (result.w == 1) {
				senpai.cache[result.id].message = 'Played this week.';
				API.chatLog("Your next song has already been played this week, we recommend choosing a different song.", true);
			} else if (senpai.pos !== undefined) {
				return;
			} else if (result.b !== 1 && result.o == 0 && result.t == 0 && result.w != 1) {
				senpai.cache[result.id].message = 'Seems OK, but check rules.';
				API.chatLog("This version of your song is not marked as banned or overplayed, but please double check it. (Other videos of the same song might be banned.) http://s.AnimeMusic.me/plug-guide Last played: " + result.w, false);
			} else {
				senpai.cache[result.id].message = 'Error! Notify Uricorn.';
				API.chatLog("Error: Please send this to Uricorn:" + result.id, true);
			}
		},

		checked: function() {
			return;
			console.log(senpai.checkedSongs[API.getNextMedia().cid]);
			if (!senpai.checkedSongs[API.getNextMedia().media.cid])
				return;

			$('.playlist-media-first-item').css('background', '#222255');
			var author = $('.playlist-media-first-item .author');
			if (!/(senpai-checked)$/.test(author.text()))
				author.text(author.text() + ' (senpai-checked)');
		},

	};
	if (!window.senpai || !window.senpai.initialized) {
		senpai.startUp();
	}
	senpai.initialized = true;
	if (window.senpai && window.senpai.cache)
