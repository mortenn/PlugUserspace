	...
	var senpai = {
		...
		manageCheckButton: function()
		{
			...
			if(window.senpai.getActivePlaylist() == window.senpai.getCurrentPlaylist())
			{
				...
				button = $('<div id="playlist-check-button" class="button" tooltip="'+tooltip+'" style="right:350px;"><span>'+_('Check')+'</span></div>');
			...
			for(var i = 0; i < targets.length; ++i)
			{
				...
				if(data)
				{
					...
					if('kouhai' in window && 'o2' in data.result && data.result.o2 > 0)
						message += ' (<a href="http://j.animemusic.me/animemusic/opcheck.php?media='+data.media.cid+'" target="_blank">OP='+data.result.o2+'</a>)';
